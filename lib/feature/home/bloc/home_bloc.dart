...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  FutureOr<void> _onHomeOnAppStartedEvent(_HomeOnAppStartedEvent event, Emitter<HomeState> emit) async {
    ...
    List<Object> data = await Future.wait(
        [marvelApi.getCharacters(0), marvelApi.getComics(0), marvelApi.getSeries(0), marvelApi.getStories(0)]);
    List<Story> stories = (data[3] as ApiResponseStory)
        .data
        .results
        .map((story) => Story(id: story.id, title: HtmlUnescape().convert(story.title)))
        .toList();
    emit(
      HomeState.loaded(
