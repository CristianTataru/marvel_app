...
void main() {
  group('''SeriesPage''', () {
    testWidgets('''Open SeriesPage and load paginated data successfully''', (tester) async {
      await homepageIsOpened(tester);
      await iTapOnWidgetAtIndex(tester, UserPick, 2);
      await iWait(tester);
      await iScrollToBottomOfPage(tester, 'seriesPageScrollKey');
      await iWait(tester);
      await iScrollToBottomOfPage(tester, 'seriesPageScrollKey');
      await iSeeText(tester, '15-Love');
    });
  });
}
